...
基于 OpenPDF 2.0.3 的 23 个功能模块对照示例，每个模块都是独立可运行的 Java 类，
与 `examples/` 下的仓颉示例一一对应（S01–S23）。
...
# 运行全部 23 个模块（无参或 all）
java -jar target/pdf-compare-1.0-SNAPSHOT-jar-with-dependencies.jar all

# 运行单个模块（编号 1..23，如 S07 表格）
java -jar target/pdf-compare-1.0-SNAPSHOT-jar-with-dependencies.jar 7
```

输出目录：`openpdf-examples/output/`
